...
impl AuthorityStore {
    ...
    pub fn multi_input_objects_available(
        ...
        for ((idx, input_key), has_key) in keys_with_version.iter().zip(
            ...
            if has_key {
                ...
                // the object exists in the object marker table as well. If so we will then mark it as
                // "available" to let it progress through.
                let has_received = self.have_received_object_at_version(
                    &input_key.id(),
    ...
    async fn update_objects_and_locks(
        ...
        // Insert each received object into the received objects marker table
        write_batch.insert_batch(
            &self.perpetual_tables.object_per_epoch_marker_table,
            received_objects.iter().map(|(object_id, version, _)| {
                (
                    (
                        epoch_id,
                        ObjectKey(*object_id, *version),
