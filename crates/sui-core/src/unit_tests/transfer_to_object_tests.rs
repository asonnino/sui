...
impl TestRunner {
    ...
    pub async fn execute_certificate(&mut self, ct: VerifiedCertificate) -> TransactionEffects {
        execute_certificate_with_execution_error(&self.authority_state, None, ct, false)
            .await
            .unwrap()
            .1
            .into_data()
...
async fn verify_tto_not_locked(flipper: bool) -> (TransactionEffects, TransactionEffects) {
    ...
    let (valid_effects, invalid_effects) = if flipper {
        let invalid_effects = runner.execute_certificate(cert_for_fake_parent).await;
        let valid_effects = runner.execute_certificate(valid_cert).await;
...
async fn test_tto_dependencies_dont_receive() {
    ...
    assert!(effects.unwrapped_then_deleted.is_empty());
    assert!(effects.wrapped.is_empty());
    // Not received, so no dependency is added.
    assert!(!effects.dependencies.contains(&transfer_digest));
    ...
    for (obj_ref, owner) in effects.mutated.iter() {
        ...
        if obj_ref.0 == parent.0 .0 {
            ...
            assert_eq!(obj_ref.1.value(), parent.0 .1.value() + 1);
        }
    }
}
