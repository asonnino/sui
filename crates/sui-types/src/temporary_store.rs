...
pub struct InnerTemporaryStore {
    ...
    pub loaded_runtime_objects: BTreeMap<ObjectID, SequenceNumber>,
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn update_with_received_objects(
        &mut self,
        receiving_objects: Vec<ObjectRef>,
        transaction_dependencies: &mut BTreeSet<TransactionDigest>,
    ) {
        // For any objects that we have received in this transaction, we need to add the
        // transaction that transferred that object as a dependency.
        let received_dependencies = receiving_objects
            .into_iter()
            .filter_map(|obj_ref| self.loaded_runtime_objects.get(&obj_ref.0));
        transaction_dependencies.extend(
            received_dependencies
                .clone()
                .map(|obj_meta| obj_meta.previous_transaction),
        );

        // For any received objects, we need to update the lamport timestamp for the transaction to be
        // the maximum of the input objects and objects that have been received in the transaction.
        // NB: this is _not_ and should not be the max of the receiving arguments of the transaction.
        self.lamport_timestamp = self
            .lamport_timestamp
            .max(SequenceNumber::lamport_increment(
                received_dependencies.map(|meta| meta.version),
            ));
    }
    ...
    pub fn to_effects(
        ...
        // Update lamport version to be 1 greater than any objects that were received in the transaction.
        // Add any received objects previous transactions as dependencies on this transaction.
        self.update_with_received_objects(receiving_objects, &mut transaction_dependencies);
